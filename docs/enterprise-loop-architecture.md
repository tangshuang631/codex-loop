...
- 任何调度、发送、验证、停止相关迁移，都必须跑 `npm run production:check`。它是投入使用前的生产化检查入口，会覆盖环境检查、长跑 smoke 检查、`npm test`、桌面端构建、移动端构建、前端证据检查和 git 差异检查，并写入可追溯报告。前端证据检查会确认桌面端和移动端构建产物仍包含历史对话、发送引导、截图证据、生产阶段、生产成熟度、剩余缺口、闭环证据、真实闭环、验证目标、启动预检、复制命令、复制文件、折叠详情、代码块和文件路径，并把结果写入 `runtime/frontend-evidence/`。
