...
- 任何调度、发送、验证、停止相关迁移，都必须跑 `npm run production:check`。它是投入使用前的生产化检查入口，会覆盖环境检查、长跑 smoke 检查、`npm test`、桌面端构建、移动端构建、前端证据检查和 git 差异检查，并写入可追溯报告。前端证据检查会确认桌面端和移动端构建产物仍包含历史对话、发送引导、截图证据、生产阶段、生产成熟度、剩余缺口、闭环证据、真实闭环、补充合并证据、下一轮验证、验证目标、启动预检、复制命令、复制文件、折叠详情、代码块和文件路径，并把结果写入 `runtime/frontend-evidence/`。
...
- 真实运行观测至少 2 轮连续闭环才算长期运行基本证据：发送下一轮指令 -> Codex 完成 -> NPC 复盘。同时还需要至少 1 次补充合并证据：用户从桌面端或移动端写入补充引导，等 Codex 完成后由本地模型 / NPC 合并进下一条指令。单轮闭环或缺少补充合并证据都只能证明可试用，不能证明适合提高自动化时长。
- `npm run production:status` 输出生产状态摘要，统一汇总最近生产检查、前端证据、长跑节奏、真实运行观测、补充合并证据和下一步建议。摘要必须区分 `readiness.stage`：`trial` 只能说明代码闸门通过且适合短时试用，`observing` 说明正在等待真实任务形成证据，`production` 才说明已经具备真实 2 轮闭环证据和补充合并证据，`blocked` 则必须先修复失败项。默认超过 12 小时的报告会被视为已过期，需要重新运行 npm run production:check 后再判断是否适合长期运行。
...
3. 让历史渲染具备 Codex 桌面端同类信息块能力：已编辑文件、已运行命令、脚本输出、测试结果、截图证据和验证日志默认只显示中文摘要、数量和状态，展开后保留可复制文件路径、命令和完整内容。
4. 继续扩展 `app/server/lib/loop-core/controller-gates.mjs`，把更多停止条件、失败分类和恢复策略收束为可单测的 Loop 内核判断模块。
5. 继续扩展 `app/server/lib/verification/supervisor-verification.mjs`，把更多测试、构建、日志、截图验收能力收束进验证层。
6. 把 NPC 复盘、用户补充合并、项目规则读取收束到 NPC 决策层。
7. 把 Codex 发送、历史读取、可见性校验收束为 Codex 联动层统一接口，并补上项目路径 + 窗口名的 thread-resolver 自动绑定。
8. 继续把启动残留状态、日志可读化、关闭服务、远程访问收束为运行治理层，并扩展扫码长期绑定的撤销、轮换和审计能力。
...
- 真实运行日志仍能解释失败原因。
- 前端仍保持中文、简洁、用户视角。
- 桌面端、`/mobile` 和 `app/mobile` 仍共用同一套历史对话模型，不能出现电脑端是对话、手机端是日志列表的割裂体验。
- 大块文件、命令、脚本、测试和截图证据默认折叠，展开后可复制关键路径和命令。
- 已配对移动端仍能查看进程、发送/编辑/撤回引导，并遵守 Codex 未完成时不打断的同一套闸门。
