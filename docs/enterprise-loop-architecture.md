...
- 任何调度、发送、验证、停止相关迁移，都必须跑 `npm run production:check`。它是投入使用前的生产化检查入口，会覆盖环境检查、长跑 smoke 检查、`npm test`、桌面端构建、移动端构建和 git 差异检查，并写入可追溯报告。
- `npm run loop:smoke` 是 Loop 内核的本地模拟长跑检查，不触碰真实 Codex 线程。它必须证明控制器会在发送后等待 Codex、不会在 Codex 未完成时追发、用户补充会等 Codex 完成后交给 NPC 合并、会先做监督复盘再进入下一轮，并在预算到达后停止自动发送。
