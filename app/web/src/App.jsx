...
function presentPendingGuidanceStatus(result, fallback = "已记录下一轮补充。") {
  ...
  const primary = presentMonitorText(result?.message).trim();
  if (primary) {
    return primary;
  }
  ...
  const statusLabel = presentMonitorText(pending.statusLabel).trim();
  const detail =
    presentMonitorText(pending.userMessage).trim() ||
    presentMonitorText(pending.statusDetail).trim();
...
function MobileTaskApp() {
  ...
  async function confirmPairing() {
    if (!effectiveSessionId || !effectivePairingCode) {
      setErrorText("请粘贴桌面端显示的绑定链接或二维码内容；如果无法自动识别，再填写会话编号和确认码。");
      return;
    }
  ...
  async function saveMobileGuidance() {
    ...
    try {
      ...
      setStatusText(
        presentMonitorText(guidanceResult?.message) ||
          (guidanceResult?.dispatch === "sent"
            ? "已发送引导，正在等待 Codex 完成当前轮。"
            : "已保存补充引导，会等 Codex 完成后交给本地模型监督流程合并。"),
      );
  ...
  async function clearMobileGuidance() {
    ...
    try {
      ...
      setStatusText(presentMonitorText(result?.message, "已撤回待合并引导。"));
    ...
    <main className="mobile-task-shell">
      ...
      {!mobileDevice ? (
        <section className="mobile-task-pairing">
          <h2>绑定这台电脑</h2>
          <p>优先用独立移动端 App 扫描桌面端二维码；当前页面可作为无法扫码时的备用绑定入口。</p>
          <textarea
            rows={4}
  ...
  const runningDescription = isFinalizing
    ...
    : isDispatching
      ...
      : isReviewing
        ...
        : "需要继续时点击开始循环；如需本地模型参与，请先在设置里开启本地模型。";
    ...
    <div className="sidebar-pane-stack">
      ...
      >
        ...
        >
          ...
          >
            <summary>监督角色</summary>
            <p className="sidebar-help">
              定义 codex-loop 默认的产品经理、测试人员、挑剔用户和监工偏好；当前任务如果有专用规则，会在这里的基础上覆盖。
            </p>
            <div className="settings-grid">
              <label>
                <span>角色特性</span>
...
function HelpWorkspaceView() {
  const guides = [
    ...
    {
      title: "如何开启本地模型增强",
      body: "进入设置，打开本地模型接入并选择模型。开启后，所有任务默认会把 Codex 回复交给本地模型整理，再生成下一轮指令。",
    },
    {
      title: "如何安全停止与关闭服务",
...
function DashboardHome({
  ...
  const modelStatus = settingsForm.promptGeneratorEnabled === "auto"
    ? "自动接入本地模型 · " + settingsForm.promptGeneratorModel
